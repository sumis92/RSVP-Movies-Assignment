...
-- ANSWER:
-- The number of rows in each table are director_mapping = 3867,genre = 14662,movie = 7902,names = 26854,ratings = 8230,role_mapping = 16037
...
-- ANSWER
-- The following columns containing Null values: 
-- 1. country
...
-- Here we can see that Number of Movies produced in March is 824
...
select country,year,COUNT(id) as number_of_movies
from movie
where 
(
	country LIKE '%USA%'
...
-- Number of movies released in India or USA for the year 2019 is 1059

-- While Number of movies produced in India

SELECT COUNT(id) AS num_of_movies_IND FROM movie
WHERE country = 'India';

-- 1007 movies produced in India.

-- While Number of movies produced in USA

SELECT COUNT(id) AS num_of_movies_USA FROM movie
WHERE country = 'USA';

-- 2260 movies produced in USA
...
-- Drama Genre has the highest number of movies, followed by Comedy and Thriller
...
-- ANSWER : 3289 movies belong to one genre.
...
-- ANSWER:
-- Here we can see that the Drama has an average duration of 106.77
...
-- ANSWER:
-- Here we can see that Thriller is among the top 3 genres
...
-- We found that the FAN is in the top 10 movies with an average rating of 9.6
...
-- Here we can see that the movie count is the highest in the median_rating of 7
...
-- Here we can see that the Dream Warrior Pictures or National Theatre Live or Both has the highest number of hits having average rating of more than 8.
...
-- Here we can see that the Drama with movie count of 24 is the highest with the above condition stated.
...
-- Here we can see that total of 361 movies released between 1 April 2018 and 1 April 2019 have a median rating of 8.
...
SELECT languages, sum(r.total_votes) as total_votes
FROM movie as m
INNER JOIN
ratings AS r
ON m.id = r.movie_id
WHERE languages LIKE '%German%' OR languages LIKE '%Ital%'
GROUP BY country
ORDER BY total_votes DESC;

-- Looking for the Total votes for German movies
SELECT
...
--  The sum of total votes for German movies is 2026223


-- -- Looking for the Total votes for Italian movies
   SELECT
   sum(total_votes) 
FROM
   movie m 
...
-- Total Votes for Italian movies is 703024

-- Here we can conclude that Germany movies got more votes than Italian movies.
...
-- Here we can conclude that 'Name' Column has no NULL values.
...
-- Here we can see that James Mangold can be hired as a Director for the next RSVP Project
...
-- The two actors that can be hired are
-- 1. Mammootty
-- 2. Mohanlal
...
-- Here we can see that Marvel Studios has a vote count of 2656967, followed by Twentieth Century Fox and Warner Bros.
...
-- Here we can see that Actor Vijay Sethupathi has the higher average rating is 8.42
...
HAVING COUNT(m.id)>=3
LIMIT 5;
-- The top actresses are Taapsee Pannu, followed by Divya Dutta, Kirti Kharbanda, Sonakshi Sinha
...
SUM(ROUND(AVG(duration),1)) OVER(ORDER BY genre ROWS UNBOUNDED PRECEDING) AS running_total_duration,
        ROUND(AVG(ROUND(AVG(duration),1)) OVER(ORDER BY genre ROWS 10 PRECEDING),2) AS moving_avg_duration
FROM movie AS m
INNER JOIN
genre AS g
ON m.id =g.movie_id
...
-- Highest five Grossing movies of each year based on the top three genres-
WITH top_3_genre AS
( 	
...
SELECT production_company,
                COUNT(m.id) AS movie_count,
                RANK() OVER(ORDER BY COUNT(id) DESC) AS prod_com_rank
FROM movie AS m 
INNER JOIN ratings AS r 
ON m.id = r.movie_id 
...
-- Here we can see that Star Cinema tops followed by Twenthieth Century Fox with Highest number of hits.
...
-- The Top three actresses with an average ratng>8 are:
-- Sangeetha Bhat  :  1
-- Fatmire Sahiti  :  2
-- Adriana Matoshi :  3
...
(
SELECT dm.name_id, name, dm.movie_id,
	   m.date_published, 
       LEAD(date_published, 1) OVER(PARTITION BY n.id ORDER BY date_published) AS next_movie_date
...
-- Here we can conclude that A.L. Vijay is the top director with maximum number of movies and other factors,followed by 
-- Andrew Jones
-- Özgür Bakar
-- Justin Price
-- Sion Sono
